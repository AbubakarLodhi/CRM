    ...
    WithEvents
{
    ...
    protected static function buildChronologicalEntries(Collection $purchases, Collection $cashFlows): array
    {
        ...
        foreach ($purchases as $purchase) {
            ...
            $invoiceDate = $purchase->purchase_date ?? $purchase->created_at;

            $entries[] = [
                'date' => $invoiceDate->copy()->startOfDay(),
                'created_at' => $purchase->created_at,
                'debit' => round((float) ($purchase->total_amount ?? 0), 2),
                'credit' => 0.0,
                'description' => self::invoiceDescription($purchase),
                'extras' => [
                    ...
                    'items_count' => (int) ($purchase->items_count ?? $purchase->items->count()),
                    'subtotal' => (float) ($purchase->subtotal ?? 0),
                    'discount' => $discount,
                    'tax' => $tax,
                    'total_amount' => round((float) ($purchase->total_amount ?? 0), 2),
                ],
            ];
            ...
            foreach (($purchase->payments ?? collect())->sortBy([['payment_date', 'asc'], ['created_at', 'asc']]) as $payment) {
                ...
                $entries[] = [
                    ...
                    'description' => self::paymentDescription($purchase->purchase_no, $payment->reference_no, $payment->method),
                    'extras' => [
                        'party_name' => (string) ($purchase->vendor?->name ?? '-'),
                        'merchant' => (string) ($purchase->merchant?->name ?? '-'),
                        'branch' => (string) ($branchText ?: '-'),
    ...
    protected static function invoiceDescription($purchase): string
    {
        $variantNames = $purchase->items
        ...
        return 'Invoice ' . (string) ($purchase->purchase_no ?? '-') . ' - ' . $variantsText;
    }

    protected static function paymentDescription(?string $purchaseNo, ?string $reference, ?string $method): string
    {
        $parts = ['Payment'];
